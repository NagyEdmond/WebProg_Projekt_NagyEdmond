...
class CurrencyDB
{
    ...
    public function getCurrencyID(string $currencyName)
    {
        $sql = "SELECT currency_id FROM currencies WHERE currency_name = ?";
        $stmt = $this->db->prepare($sql);
        $stmt->bind_param('s', $currencyName);
    ...
    public function getCurrencyValues($currencyName)
    {
        $id = $this->getCurrencyID($currencyName);

        $sql    = "SELECT rate FROM currency_rates WHERE currency_id = ? ORDER BY currency_id";
        $stmt = $this->db->prepare($sql);
    ...
    public function getAllDates()
    {
        ...
        $rows = [];
        while ($row = $result->fetch_assoc()) {
            $rows[] = $row['date_of_rate'];
        }

        return $rows;
    }

    public function getLatestRate($currencyName)
    {
        $id = $this->getCurrencyID($currencyName);
        $sql = "SELECT rate FROM currency_rates WHERE currency_id = ? ORDER BY date_of_rate DESC LIMIT 1";
        $stmt = $this->db->prepare($sql);
        $stmt->bind_param('s', $id);
        $stmt->execute();
        $result = $stmt->get_result();
